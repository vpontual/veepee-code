...
- **Self-measurement** -- `--eval` scores the harness against graded coding tasks ([Harness Evaluation](harness-eval.md)), `--improve` turns those results into a gated, reviewable branch that is never merged or deployed ([Self-Improvement](self-improvement.md)), and the agent injects [turn nudges](nudges.md) to catch work that stopped just short of done.
